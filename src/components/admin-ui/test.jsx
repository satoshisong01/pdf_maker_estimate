...
const Test = (props) => {
    ...
    const onsenddata = (event) =>{
        setTablename(event.Tables_in_test);
        let body ={
            content_name : event.Tables_in_test,  
        } 
    ...
    const paintdata = (array) => {
        return array.map((item, index) => (
            <tr key={index}>
                ...
                <th>
                    <form>
                        <Link to={`/update/${item.idx}/${tablename}/${item.name}/${item.price}/${item.content}`}>
                            <button type="click">
                                수정
                            </button>
                        </Link>
    ...
    const list_Array =  (array) => {
        return array.map((item, index) =>(
            <Menu.Item onClick={(event) => {onsenddata(item, event)}} key={index} icon={<PieChartOutlined/>}>{item['Tables_in_test']}</Menu.Item>
        ))
    }
