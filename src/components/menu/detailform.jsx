...
function Detailform (){
    ...
    const service_platforms = () => {
        axios.get("/api/service_platform")
            .then(res =>{
    ...
    const document_extractions = () => {
        axios.get("/api/document_extraction")
            .then(res =>{
    ...
    const etcs = () => {
        ...
        console.log(mailsend_Service,"나와라")
                    ...
                    <div className="content" key={index}>
                        <label onChange={(e) => onChangeHandlerRadioUi(item, e)}>
                            <div className="mm_1">
                                <div className="main_title">
                                    <h5 className="content-name">{item.name}</h5>
                                    <input name="radio-btn" className="checkbox" id={item.idx} type="radio"/>
                                </div>
                                    <h4 className="content-price">{(item.price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}원</h4>
                                    <span className="content-comment">{item.content}</span>
                            </div>
    ...
    return (
        <>
            <div className="box-blocks">
                <Sendinof arrui={mailsend_UI} arrservice ={mailsend_Service} arrtotal ={arr} sendmailall = {mailsendAll} sendmailname ={mailsendname} sendmailcontent={mailsendcontent} pricedui = {priceui} pricedtotal = {priced} priceservice ={priceservice}/>
                <div className="item-lists">
                    <div className="caution">
                    * 예상 견적은 고객사가 구현하고자 하는 기능들을 알아보기 위한 수단입니다.<br></br>
                    * 개발문의 후 시스너가 정확한 견적 산출을 위해 도움을 드리니, 희망하시는 기능을 추가해보세요.</div>
                    ...
                    <h1 className="main-form-title">고급 기능</h1>
                        <div className="item-list">
                            {arrayMap(advance_action)}
                        </div>
                    <h1 className="main-form-title">서비스를 운영할 서버 플랫폼</h1>
                        <div className="item-list">
                            {serviceMap(service_platform)}
                        </div>
                    ...
                    <h1 className="main-form-title">외부 API 및 크롤링 관련</h1>
                        <div className="item-list">
                            {arrayMap(api_crawling)}
                        </div>
                    <h1 className="main-form-title">문서 추출 관련</h1>
                        <div className="item-list">
                            {arrayMap(document_extraction)}
                        </div>
